...
for (i in seq_len(nrow(test_rows))) {
  ...
  # Replicate Step 4's precedent: restrict to labeled-cell subset
  labeled <- !is.na(tg) & tg != "" 
  ...
  set.seed(42)
  km_nstart1  <- kmeans(emb_l, centers = k)$cluster
  set.seed(42)
  km_nstart25 <- kmeans(emb_l, centers = k, nstart = 25)$cluster

  ari_1  <- aricode::ARI(km_nstart1, tg_l)
  ari_25 <- aricode::ARI(km_nstart25, tg_l)
