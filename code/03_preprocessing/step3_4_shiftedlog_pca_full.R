...
OUT_ROOT     <- "data/processed/pca_shiftedlog"
LOG_FILE     <- file.path(OUT_ROOT, "step3_4_progress.csv")
...
run_shiftedlog_pca_one <- function(sce_path, out_dir, flags = list()) {
  out_file <- file.path(out_dir, paste0(tools::file_path_sans_ext(basename(sce_path)), "_shiftedlogpca.rds"))
  ...
  res <- tryCatch({
    ...
    safe_lib_sizes[safe_lib_sizes == 0] <- 1
    ...
    # Sparse shortcut: centered PCA of log(x+delta) == centered PCA of S,
    # where S holds log(x+delta)-log(delta) only at originally-nonzero
    # positions (zero elsewhere) -- exact, verified against dense computation
    S <- normalized
    S@x <- log(S@x + delta_opt) - log(delta_opt)

    n_cells <- ncol(S); n_genes <- nrow(S)
    nv <- min(PCA_NV, n_cells - 1, n_genes - 1)

    set.seed(SEED)
    pca_fit <- irlba(t(S), nv = nv, center = TRUE, scale = FALSE)
    ...
    out <- list(
      ...
      nv_used = nv,
      seed = SEED,
      method = "irlba_shiftedlog_pca",
      target_sum = TARGET_SUM,
      delta_used = delta_opt,
      delta_type = "data_driven_optimal_median_nonzero",
      centered = TRUE,
    ...
    rm(sce, counts_mat, normalized, S, pca_fit, embedding); gc(verbose = FALSE)
...
for (name in names(real_files)) {
  counter <- counter + 1
  run_shiftedlog_pca_one(real_files[[name]], file.path(OUT_ROOT, "real", name))
...
for (sim_name in names(sim_files)) {
  ...
  for (i in seq_along(files)) {
    counter <- counter + 1
    run_shiftedlog_pca_one(files[i], out_dir)
