...
OUT_ROOT     <- "data/processed/pca_log"
LOG_FILE     <- file.path(OUT_ROOT, "step3_3_progress.csv")
...
run_log_pca_one <- function(sce_path, out_dir, flags = list()) {
  out_file <- file.path(out_dir, paste0(tools::file_path_sans_ext(basename(sce_path)), "_logpca.rds"))
  ...
  res <- tryCatch({
    ...
    logged <- log1p(normalized)  # sparsity-preserving: log1p(0) = 0

    n_cells <- ncol(logged); n_genes <- nrow(logged)
    nv <- min(PCA_NV, n_cells - 1, n_genes - 1)

    set.seed(SEED)
    pca_fit <- irlba(t(logged), nv = nv, center = TRUE, scale = FALSE)
    ...
    rm(sce, counts_mat, normalized, logged, pca_fit, embedding); gc(verbose = FALSE)
...
for (name in names(real_files)) {
  counter <- counter + 1
  run_log_pca_one(real_files[[name]], file.path(OUT_ROOT, "real", name))
...
for (sim_name in names(sim_files)) {
  ...
  for (i in seq_along(files)) {
    counter <- counter + 1
    run_log_pca_one(files[i], out_dir)
