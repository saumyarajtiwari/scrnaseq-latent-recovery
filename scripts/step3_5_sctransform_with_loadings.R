...
BATCH_SIZE   <- 200
N_WORKERS    <- 4
OUT_ROOT     <- "data/processed/pca_sctransform_v2"
LOG_FILE     <- file.path(OUT_ROOT, "step3_5_v2_progress.csv")
...
process_one <- function(sce_path, out_dir) {
  ...
  res <- tryCatch({
    ...
    # Fix 1: exclude genuinely native zero-total-count cells (found in Splatter's raw output)
    ...
    if (n_genes_orig > MAX_GENES) {
      ...
      # Fix 2: rescue genes for cells that become all-zero under the HVG subset
    ...
    rownames(loadings) <- rownames(residuals)
    ...
    out <- list(
      ...
      method = "sctransform_v2_pearson_pca", latent_var = "log_umi",
    ...
    rm(sce, counts_mat, vst_out, residuals, pca_fit, embedding, loadings, out)
    gc(verbose = FALSE)
    list(timestamp = as.character(Sys.time()), file_path = sce_path, out_path = out_file,
         status = "done", nv_used = nv, runtime_sec = NA, msg = "")
  }, error = function(e) {
    gc(verbose = FALSE)
    list(timestamp = as.character(Sys.time()), file_path = sce_path, out_path = out_file,
...
cat("\n=== STEP 3.5 (WITH LOADINGS) COMPLETE ===\n")
